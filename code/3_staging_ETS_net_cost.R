...
create_plot <- function(
    title, dataset, x_field, x_name, y_field, y_name, group_field, group_name,
    width, height
) {
  ...
  ggsave(
    filename = here::here("plots", file_name),
    plot = plot,
    width = width,
    height = height,
    units = "in",
...
# rename main_activity_code to ets_activity_code for uniform naming throughout the script
ets_allocated_fully_up_to_2024 <- readxl::read_excel("data/1_raw/ETS prices/ets_allocated_up_to_2024.xlsx") %>% 
  rename(ets_activity_code = main_activity_code)

unique(ets_allocated_fully_up_to_2024$country_code)
...
unique(ets_allocated_fully_up_to_2024$active_installation)
unique(ets_allocated_fully_up_to_2024$citl_information)
unique(ets_allocated_fully_up_to_2024$unit)
unique(ets_allocated_fully_up_to_2024$year)


activity_code_groups = c("20-99", "21-99")

# import the labels of the ets activity_codes
ets_activity_code_labels <- read_excel("data/1_raw/mapping ets nace/mapping_ets_nace_level3.xlsx",
                                       sheet="Mapping") %>%
  transmute(
...
ets_freely_allocated_by_year_coun_acti <- ets_allocated_fully_up_to_2024 %>%
  filter(
    ...
    !ets_activity_code %in% activity_code_groups
  ) %>%
  mutate(year = as.integer(year)) %>%
  group_by(country_code, year, ets_activity_code) %>%
  summarise(
...
ets_corrections_by_year_coun_acti <- ets_allocated_fully_up_to_2024 %>%
  filter(
    !country_code %in% exclude_non_country_codes,
...
ets_freely_allocated_corr_by_year_coun_acti <- ets_freely_allocated_by_year_coun_acti %>%
  left_join(
    ...
    by = c("country_code", "year", "ets_activity_code")
  ) %>%
  mutate(
...
writexl::write_xlsx(
  ...
  "data/2_staging/ets_freely_allocated_corr.xlsx"
)
...
ets_freely_allocated_corr_by_year_coun_acti_labeled <- 
  ...
  left_join(
    ...
    by = c("ets_activity_code" = "ets_activity_code")
  )
...
top_allocated_activities <- ets_freely_allocated_corr_by_year_coun_acti_labeled %>%
  group_by(ets_activity_code, ets_activity_name) %>%
  summarise(
...
create_plot("Freely allocated allowances by activity over years", ets_freely_allocated_corr_by_year_acti_labeled_10, year, "Year", total_freely_allocated, "Freely allocated allowances tCO2 equi.", ets_activity_name, "Activity name", width = 12, height = 7)
...
#     by = c("ets_activity_code" = "ets_activity_code")
#   ) %>%
#   distinct(ets_activity_code) %>%
#   arrange(ets_activity_code)
# 
# codes_in_labels_not_in_emissions <- ets_activity_code_labels %>%
#   mutate(ets_activity_code = as.character(ets_activity_code)) %>%
#   distinct(ets_activity_code, ets_activity_name) %>%
#   anti_join(
#     ets_emissions_by_year_coun_acti %>%
#       mutate(ets_activity_code = as.character(ets_activity_code)) %>%
#       distinct(ets_activity_code),
...
writexl::write_xlsx(
  ...
  "data/2_staging/ets_emissions.xlsx"
)
...
# plot
create_plot("Emissions by main activity over years ex. fuel combustion", ets_emissions_by_year_acti_labeled_10, year, "Year", total_emissions_tonne_CO2_equi, "Emissions tCO2", ets_activity_name, "Activity name", width = 12, height = 7)
...
ets_net_cost_by_year_coun_acti_labeled <- ets_net_cost_by_year_coun_acti %>%
  ...
  left_join(
    ets_activity_code_labels %>%
      mutate(ets_activity_code = as.character(ets_activity_code)) %>%
      distinct(ets_activity_code, .keep_all = TRUE),
    by = c("ets_activity_code" = "ets_activity_code")
  )
...
ets_net_cost_by_year_acti <- ets_net_cost_by_year_coun_acti_labeled %>%
  ...
  summarise(
    ...
    total_net_ets_cost_eur =
      sum(ets_net_cost_eur, na.rm = TRUE),
...
create_plot("Net ETS cost by activity over years", ets_net_cost_by_year_acti_10, year, "Year", total_net_ets_cost_eur, "Net ETS cost €", ets_activity_name, "Activity name", width = 12, height = 7)
